...
#define IOCTL_BK_REGISTER_PROCESS_INSTRUMENTATION_CALLBACK \
    CTL_CODE(FILE_DEVICE_BK, 0x815, METHOD_BUFFERED, FILE_WRITE_DATA)
...
// Controller IPC hint: keep the subscription in user-mode routing only.
// Kernel stream matching still uses the low BK_STREAM_* bits.
#define BK_STREAM_USERMODE_ONLY 0x80000000u
...
#define BK_DIAGNOSTIC_MAX_EVENTS 256
#define BK_DIAGNOSTIC_SCHEMA_VERSION 3u
...
#define BK_PROCESS_INSTRUMENTATION_CALLBACK_FLAG_X64 0x00000001u
#define BK_PROCESS_INSTRUMENTATION_CALLBACK_FLAG_SR71 0x00000002u

typedef struct _BK_REGISTER_PROCESS_INSTRUMENTATION_CALLBACK_REQUEST
{
    UINT32 ProcessId;
    UINT32 Flags;
    UINT64 CallbackAddress;
    UINT64 CallbackSize;
} BK_REGISTER_PROCESS_INSTRUMENTATION_CALLBACK_REQUEST, *PBK_REGISTER_PROCESS_INSTRUMENTATION_CALLBACK_REQUEST;
