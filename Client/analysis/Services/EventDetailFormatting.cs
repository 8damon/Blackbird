...
namespace BlackbirdInterface
{
    internal static class EventDetailFormatting
    {
            ...
            string? target)
        {
            ...
            {
                uint infoClass = (uint)FirstU64(fields, "a1", "c1");
            ...
            if (api.Equals("NtQueryVirtualMemory", StringComparison.OrdinalIgnoreCase))
            {
                ...
                ulong length = FirstU64(fields, "a4", "c4");
                return $"{actorText} queries virtual memory on {targetText}: {DescribeMemoryInformationClass(infoClass)} len=0x{length:X}";
            }

            if (api.Equals("NtQuerySystemInformation", StringComparison.OrdinalIgnoreCase) ||
                api.Equals("NtQuerySystemInformationEx", StringComparison.OrdinalIgnoreCase))
            {
                uint infoClass = (uint)FirstU64(fields, "a0", "c0", "systemInformationClass");
                ulong inputLength = FirstU64(fields, "a2", "c2");
                ulong outputLength = FirstU64(fields, "a4", "c4", "systemInformationLength");
                string label = DescribeSystemInformationClass(infoClass);
                if (api.Equals("NtQuerySystemInformationEx", StringComparison.OrdinalIgnoreCase))
                {
                    return $"{actorText} queries system info: {label} inLen=0x{inputLength:X} outLen=0x{outputLength:X}";
                }
        ...
        private static string DescribeProcessInformationClass(uint value)
        {
            string label = value switch
            {
                ...
                51 => "ProcessSubsystemInformation",
        ...
        {
            if (!IsBlackbirdEtwSource(view) || view.Family != BlackbirdNative.IpcEtwFamilyUserHook)
            {
                return false;
            }
