...
internal static class OperatorIdentityStore
{
    ...
    private static readonly byte[] IdentityEntropy = "Blackbird.OperatorIdentity.v1"u8.ToArray();
    private static readonly byte[] ProtectedKeyHeader = "BBOPKEY1"u8.ToArray();
    ...
    public static OperatorIdentity LoadOrCreate()
    {
        try
        {
            ...
            {
                byte[] privateBlob = ReadPrivateKeyBlob(PrivateKeyPath);
                byte[] publicBlob;
        ...
        WritePrivateKeyBlob(PrivateKeyPath, createdPrivateBlob);
    ...
    private static byte[] ReadPrivateKeyBlob(string path)
    {
        byte[] bytes = File.ReadAllBytes(path);
        if (bytes.Length > ProtectedKeyHeader.Length &&
            bytes.AsSpan(0, ProtectedKeyHeader.Length).SequenceEqual(ProtectedKeyHeader))
        {
            byte[] protectedBlob = bytes.AsSpan(ProtectedKeyHeader.Length).ToArray();
            return ProtectedData.Unprotect(protectedBlob, IdentityEntropy, DataProtectionScope.CurrentUser);
        }

        WritePrivateKeyBlob(path, bytes);
        return bytes;
    }

    private static void WritePrivateKeyBlob(string path, byte[] privateBlob)
    {
        byte[] protectedBlob = ProtectedData.Protect(privateBlob, IdentityEntropy, DataProtectionScope.CurrentUser);
        byte[] output = new byte[ProtectedKeyHeader.Length + protectedBlob.Length];
        Buffer.BlockCopy(ProtectedKeyHeader, 0, output, 0, ProtectedKeyHeader.Length);
        Buffer.BlockCopy(protectedBlob, 0, output, ProtectedKeyHeader.Length, protectedBlob.Length);
