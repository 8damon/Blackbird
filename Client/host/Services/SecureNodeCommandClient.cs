...
internal sealed class SecureNodeCommandClient : IAsyncDisposable
{
    ...
    public async Task<RemoteAnalysisResult> UploadFileForAnalysisAsync(string localPath, CancellationToken cancellationToken = default)
    {
        ...
        if (!fileInfo.Exists)
        {
            throw new FileNotFoundException("The selected file was not found.", localPath);
        }
        ...
        byte[] buffer = new byte[UploadChunkBytes];
        int chunkIndex = 0;
        using var stream = new FileStream(localPath, FileMode.Open, FileAccess.Read, FileShare.Read, UploadChunkBytes, FileOptions.SequentialScan);
        while (true)
        {
            ...
            CommandResponse chunkResponse = await SendCommandAsync(
                ...
                JsonSerializer.SerializeToElement(new UploadChunkPayload
                {
                    ...
                    ChunkBase64 = Convert.ToBase64String(buffer, 0, read)
                }, JsonOptions),
                cancellationToken);
