...
#define BLACKBIRD_CONTROLLER_MANUAL_MAP_PROBE_QUEUE_CAPACITY 128u

typedef struct _BLACKBIRD_CONTROLLER_MANUAL_MAP_PROBE_REQUEST
{
    BOOL Active;
    DWORD ActorPid;
    DWORD TargetPid;
...
} BLACKBIRD_CONTROLLER_MANUAL_MAP_PROBE_REQUEST, *PBLACKBIRD_CONTROLLER_MANUAL_MAP_PROBE_REQUEST;

static CRITICAL_SECTION g_ManualMapProbeLock;
static BOOL g_ManualMapProbeLockInitialized = FALSE;
static HANDLE g_ManualMapProbeEvent = NULL;
static HANDLE g_ManualMapProbeThread = NULL;
static BLACKBIRD_CONTROLLER_MANUAL_MAP_PROBE_REQUEST
    g_ManualMapProbeQueue[BLACKBIRD_CONTROLLER_MANUAL_MAP_PROBE_QUEUE_CAPACITY];
...
static BOOL ControllerManualMapProbeDequeue(_Out_ PBLACKBIRD_CONTROLLER_MANUAL_MAP_PROBE_REQUEST Request)
{
    DWORD i;

    if (!g_ManualMapProbeLockInitialized || Request == NULL)
    {
        return FALSE;
    }

    EnterCriticalSection(&g_ManualMapProbeLock);
    for (i = 0; i < RTL_NUMBER_OF(g_ManualMapProbeQueue); ++i)
    {
        ...
        *Request = g_ManualMapProbeQueue[i];
        ...
        LeaveCriticalSection(&g_ManualMapProbeLock);
        return TRUE;
    }
    LeaveCriticalSection(&g_ManualMapProbeLock);
    return FALSE;
...
{
    DWORD i;
    DWORD freeIndex = BLACKBIRD_CONTROLLER_MANUAL_MAP_PROBE_QUEUE_CAPACITY;
    DWORD oldestIndex = BLACKBIRD_CONTROLLER_MANUAL_MAP_PROBE_QUEUE_CAPACITY;
    ULONGLONG oldestTick = ~0ull;
    ULONGLONG nowTick = GetTickCount64();

    if (!g_ManualMapProbeLockInitialized || g_ManualMapProbeEvent == NULL || TargetPid == 0)
    {
        return;
    }
    ...
    for (i = 0; i < RTL_NUMBER_OF(g_ManualMapProbeQueue); ++i)
    {
        ...
        if (g_ManualMapProbeQueue[i].ActorPid == ActorPid && g_ManualMapProbeQueue[i].TargetPid == TargetPid)
        {
            ...
            LeaveCriticalSection(&g_ManualMapProbeLock);
            (void)SetEvent(g_ManualMapProbeEvent);
            return;
        }
                                                ...
                                                _In_ ULONGLONG SizeHint)
{
    ...
    if (lastProbeTick != 0 && (nowTick - lastProbeTick) < BLACKBIRD_CONTROLLER_MANUAL_MAP_PROBE_MIN_INTERVAL_MS &&
    ...
    if (entry != NULL)
    {
        if (entry->LastManualMapProbeTick != 0 &&
            (nowTick - entry->LastManualMapProbeTick) < BLACKBIRD_CONTROLLER_MANUAL_MAP_PROBE_MIN_INTERVAL_MS &&
            entry->LastManualMapProbeBase == candidateBase &&
            (entry->LastManualMapProbeSize == 0 || candidateSize == 0 || entry->LastManualMapProbeSize == candidateSize))
        {
            entry = NULL;
        }
        else
...
{
    if (g_ManualMapProbeThread != NULL)
    {
        return TRUE;
    }

    if (!g_ManualMapProbeLockInitialized)
    {
        InitializeCriticalSection(&g_ManualMapProbeLock);
        g_ManualMapProbeLockInitialized = TRUE;
    }

    ZeroMemory(g_ManualMapProbeQueue, sizeof(g_ManualMapProbeQueue));
    g_ManualMapProbeEvent = CreateEventW(NULL, FALSE, FALSE, NULL);
    if (g_ManualMapProbeEvent == NULL)
    {
        return FALSE;
    }

    g_ManualMapProbeThread = CreateThread(NULL, 0, ControllerManualMapProbeThreadProc, NULL, 0, NULL);
    if (g_ManualMapProbeThread == NULL)
    {
        CloseHandle(g_ManualMapProbeEvent);
        g_ManualMapProbeEvent = NULL;
        return FALSE;
    }
...
VOID ControllerStopHollowingWorkers(VOID)
{
    ...
    if (g_ManualMapProbeThread != NULL)
    {
        (void)WaitForSingleObject(g_ManualMapProbeThread, 3000);
        CloseHandle(g_ManualMapProbeThread);
        g_ManualMapProbeThread = NULL;
    }
    ...
    if (g_ManualMapProbeLockInitialized)
    {
        ...
        ZeroMemory(g_ManualMapProbeQueue, sizeof(g_ManualMapProbeQueue));
        LeaveCriticalSection(&g_ManualMapProbeLock);
        DeleteCriticalSection(&g_ManualMapProbeLock);
        g_ManualMapProbeLockInitialized = FALSE;
    }
}
