...
_Success_(return) static BOOL
    ControllerCreateSharedRing(_In_ DWORD Capacity, _In_ DWORD RecordSize, _Out_ HANDLE *MappingHandle,
...
_Success_(return) static BOOL
    ControllerClientIsPrivileged(_In_ const BLACKBIRD_CONTROLLER_CLIENT *Client, _Out_ BOOL *IsPrivileged)
{
    HANDLE process = NULL;
    HANDLE token = NULL;
    BOOL isAdmin = FALSE;
                                                 ...
                                                 _In_ UINT32 SampleSize)
{
    ...
    CopyMemory(buffer, Sample,
               (SampleSize < BLACKBIRD_IPC_MAX_HOOK_DATA_SAMPLE) ? SampleSize : BLACKBIRD_IPC_MAX_HOOK_DATA_SAMPLE);
                                              ...
                                              _In_ const BLACKBIRD_IPC_HOOK_EVENT *HookEvent)
{
    ...
    else
    {
        ...
        else if (HookEvent->Kind == BlackbirdIpcHookEventNt && lstrcmpiA(apiName, "NtProtectVirtualMemory") == 0)
        {
            ...
            (void)StringCchPrintfW(
                mapped.Reason, RTL_NUMBER_OF(mapped.Reason),
                L"memory.protect base=0x%llX size=0x%llX newProtect=0x%X newProtectName=%S protectFlips=%lu syscallStubOutsideNtdll=%u sampleBytes=%lu sampleBase=0x%llX",
                ...
                (unsigned int)hasSyscallStubOutsideNtdll, (unsigned long)sampleSize,
        ...
        else if (HookEvent->Kind == BlackbirdIpcHookEventModule)
        {
            ...
            if (lstrcmpiA(apiName, "RtlAddFunctionTable") == 0)
            {
                mapped.Severity = 5u;
                (void)StringCchCopyA(mapped.DetectionName, RTL_NUMBER_OF(mapped.DetectionName),
            ...
            else if (lstrcmpiA(apiName, "RtlInstallFunctionTableCallback") == 0)
            {
            ...
            else if (lstrcmpiA(apiName, "RtlDeleteFunctionTable") == 0)
            {
                mapped.Severity = 2u;
                (void)StringCchCopyA(mapped.DetectionName, RTL_NUMBER_OF(mapped.DetectionName),
                                     "USERMODE_FUNCTION_TABLE_ACTIVITY");
                (void)StringCchPrintfW(mapped.Reason, RTL_NUMBER_OF(mapped.Reason),
                                       L"functionTable.delete table=0x%llX",
            ...
            else
            {
                ...
                else if (lstrcmpiA(apiName, "LoadLibraryExA") == 0 || lstrcmpiA(apiName, "LoadLibraryExW") == 0)
                {
                    ...
                    (void)StringCchPrintfW(
                        ...
                        apiName, (nameBuffer[0] != L'\0') ? nameBuffer : L"<unknown>",
                        (unsigned long long)moduleHandle, (unsigned long long)frontFlags,
                        (unsigned long long)auxValue, (unsigned long long)HookEvent->Caller);
                }
                else
                {
                    mapped.Severity = 1u;
                                               ...
                                               _Out_ BLACKBIRD_IPC_SET_USER_HOOK_TARGET_RESPONSE *Response)
{
    ...
    ControllerLog("[IPC] set-user-hook-target mode=%lu flags=0x%08lX pid=%lu image=%ws\n", Request->Mode,
                  Request->Flags, Request->ProcessId, Request->ImagePath);
                                           ...
                                           _Out_ BLACKBIRD_IPC_PACKET *Response)
{
    ...
    {
    case BlackbirdIpcCommandHandshake:
        if (Client->Role == BlackbirdControllerClientRoleHook)
        {
            (void)InterlockedOr(&Client->HookReadyMask, (LONG)BLACKBIRD_IPC_HOOK_READY_FLAG_IPC_CONNECTED);
            Client->HookReadyTick = GetTickCount64();
        }
