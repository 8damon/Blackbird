...
static volatile LONG g_NodeIdentityLockInitialized = 0;
...
static CRITICAL_SECTION g_NodeIdentityLock;
...
{
    if (InterlockedCompareExchange(&g_NodeIdentityLockInitialized, 1, 0) == 0)
    {
        InitializeCriticalSection(&g_NodeIdentityLock);
    }
...
static BOOL ControllerNodeEnsureIdentityLoaded(VOID)
{
    ...
    EnterCriticalSection(&g_NodeIdentityLock);
