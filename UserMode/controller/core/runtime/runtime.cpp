...
{
    BLACKBIRD_CONTROLLER_CLIENT *ptr = nullptr;
    ...
    BLACKBIRD_CONTROLLER_CLIENT *release() noexcept
    {
        auto *p = ptr;
        ptr = nullptr;
        return p;
...
static DWORD WINAPI ControllerServerThreadProc(_In_ LPVOID Context)
{
    ...
    while (!ControllerShouldStop())
    {
        ...
        guard.ptr = (BLACKBIRD_CONTROLLER_CLIENT *)calloc(1, sizeof(*guard.ptr));
        if (guard.ptr == NULL)
        {
            CloseHandle(pipe);
            continue;
        ...
        guard.ptr->SlotIndex = BLACKBIRD_CONTROLLER_INVALID_SLOT;
        ...
        guard.ptr->IoctlNodeSlab = (PBLACKBIRD_CONTROLLER_EVENT_NODE)calloc(BLACKBIRD_CONTROLLER_MAX_CLIENT_QUEUE_DEPTH,
                                                                            sizeof(BLACKBIRD_CONTROLLER_EVENT_NODE));
        if (guard.ptr->IoctlNodeSlab != NULL)
        {
            for (DWORD i = 0; i < BLACKBIRD_CONTROLLER_MAX_CLIENT_QUEUE_DEPTH - 1; i++)
            {
                guard.ptr->IoctlNodeSlab[i].Next = &guard.ptr->IoctlNodeSlab[i + 1];
            }
            guard.ptr->IoctlNodeSlab[BLACKBIRD_CONTROLLER_MAX_CLIENT_QUEUE_DEPTH - 1].Next = NULL;
            guard.ptr->IoctlNodeFreeHead = &guard.ptr->IoctlNodeSlab[0];
        }

        guard.ptr->EtwNodeSlab = (PBLACKBIRD_CONTROLLER_ETW_EVENT_NODE)calloc(
            BLACKBIRD_CONTROLLER_MAX_CLIENT_ETW_QUEUE_DEPTH, sizeof(BLACKBIRD_CONTROLLER_ETW_EVENT_NODE));
        if (guard.ptr->EtwNodeSlab != NULL)
        {
            for (DWORD i = 0; i < BLACKBIRD_CONTROLLER_MAX_CLIENT_ETW_QUEUE_DEPTH - 1; i++)
            {
                guard.ptr->EtwNodeSlab[i].Next = &guard.ptr->EtwNodeSlab[i + 1];
            }
            guard.ptr->EtwNodeSlab[BLACKBIRD_CONTROLLER_MAX_CLIENT_ETW_QUEUE_DEPTH - 1].Next = NULL;
        ...
        thread = CreateThread(NULL, 0, ControllerClientThreadProc, guard.ptr, 0, NULL);
        if (thread == NULL)
        {
...
{
    const DWORD teardownMask = BLACKBIRD_RUNTIME_FLAG_SELF_HIDE | BLACKBIRD_RUNTIME_FLAG_INTERFACE_PROTECTED_ACCESS |
    ...
    if (g_DriverHandle != INVALID_HANDLE_VALUE)
    {
        if (!BLACKBIRDSCSetRuntimeConfig(g_DriverHandle, 0, teardownMask))
        {
            ControllerLog("[DRIVER][WARN] teardown runtime disarm failed (%lu)\n", GetLastError());
        }
        else
        {
            ControllerLog("[DRIVER] teardown runtime disarmed\n");
        }
