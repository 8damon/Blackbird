...
{
    // 0x00-0x05: ADD
    F_MODRM, F_MODRM, F_MODRM, F_MODRM, F_IMM8, F_IMM32,
    // 0x06-0x07: PUSH/POP ES (invalid in 64-bit) — treat as unknown
    F_INV, F_INV,
    // 0x08-0x0D: OR
    F_MODRM, F_MODRM, F_MODRM, F_MODRM, F_IMM8, F_IMM32,
    // 0x0E-0x0F: PUSH CS (invalid) / escape
    F_INV, F_INV,   // 0x0F handled separately as 2-byte escape
    ...
    F_INV, F_INV, F_INV, F_INV, F_INV, F_INV, F_INV, F_INV,
    F_INV, F_INV, F_INV, F_INV, F_INV, F_INV, F_INV, F_INV,

    // 0x50-0x57: PUSH rAX..rDI
    F_NONE, F_NONE, F_NONE, F_NONE, F_NONE, F_NONE, F_NONE, F_NONE,
    // 0x58-0x5F: POP rAX..rDI
    F_NONE, F_NONE, F_NONE, F_NONE, F_NONE, F_NONE, F_NONE, F_NONE,

    // 0x60-0x67: PUSHA/POPA/BOUND etc. (invalid in 64-bit) + addr/operand size prefixes
    F_INV, F_INV, F_INV, F_MODRM,  // 0x63 = MOVSXD r64, r/m32 (common in prologues)
    F_INV, F_INV, F_INV, F_INV,

    // 0x68: PUSH imm32; 0x69: IMUL r,r/m,imm32; 0x6A: PUSH imm8; 0x6B: IMUL r,r/m,imm8
    F_IMM32, F_MODRM|F_IMM32, F_IMM8, F_MODRM|F_IMM8,
    // 0x6C-0x6F: INS/OUTS — not in prologues
    F_INV, F_INV, F_INV, F_INV,

    // 0x70-0x7F: Jcc short (rel8)
    F_REL8, F_REL8, F_REL8, F_REL8, F_REL8, F_REL8, F_REL8, F_REL8,
    F_REL8, F_REL8, F_REL8, F_REL8, F_REL8, F_REL8, F_REL8, F_REL8,
    ...
    F_INV, F_INV, F_INV, F_INV,   // not common in prologues; skip
    // 0xA4-0xA7: MOVS/CMPS
    F_NONE, F_NONE, F_NONE, F_NONE,
    // 0xA8-0xA9: TEST AL/rAX, imm
    F_IMM8, F_IMM32,
    // 0xAA-0xAF: STOS/LODS/SCAS
    F_NONE, F_NONE, F_NONE, F_NONE, F_NONE, F_NONE,
...
static const UCHAR g_Op2[256] =
{
    ...
    F_MODRM, F_MODRM, F_MODRM, F_MODRM,  // 0x00-0x03
    F_INV, F_INV, F_INV, F_INV,           // 0x04-0x07
    F_NONE, F_NONE, F_INV, F_NONE,        // 0x08 INVD, 0x09 WBINVD, 0x0B UD2
    F_INV, F_INV, F_MODRM, F_MODRM,       // 0x0E, 0x0F, 0x0D NOP(r/m), 0x0E FEMMS

    // 0x10-0x17: SSE MOV variants
    F_MODRM, F_MODRM, F_MODRM, F_MODRM,
    F_MODRM, F_MODRM, F_MODRM, F_INV,
    // 0x18-0x1F: PREFETCH/HINT NOP
    F_MODRM, F_MODRM, F_MODRM, F_MODRM,
    F_MODRM, F_MODRM, F_MODRM, F_MODRM,  // 0x1F multibyte NOP (common compiler output)

    // 0x20-0x27: MOV CR/DR
    F_MODRM, F_MODRM, F_MODRM, F_MODRM, F_MODRM, F_MODRM, F_INV, F_INV,
    // 0x28-0x2F: SSE
    ...
    F_NONE, F_NONE, F_NONE, F_NONE, F_NONE, F_NONE, F_INV, F_INV,
    F_INV,  F_INV,  F_INV,  F_INV, F_INV,  F_INV,  F_INV, F_INV,
    ...
    F_MODRM, F_MODRM, F_MODRM, F_NONE,
    F_MODRM|F_IMM8, F_MODRM|F_IMM8, F_MODRM|F_IMM8, F_MODRM|F_IMM8,
    F_MODRM, F_MODRM, F_MODRM, F_MODRM,

    // 0x80-0x8F: Jcc near (rel32)
    F_REL32, F_REL32, F_REL32, F_REL32, F_REL32, F_REL32, F_REL32, F_REL32,
    F_REL32, F_REL32, F_REL32, F_REL32, F_REL32, F_REL32, F_REL32, F_REL32,
    ...
    // 0xA0-0xAF
    F_NONE, F_NONE, F_NONE, F_MODRM,   // PUSH FS, POP FS, CPUID, BT
    F_MODRM|F_IMM8, F_MODRM, F_MODRM, F_MODRM,  // SHLD imm8, SHLD CL, -, -
    F_NONE, F_NONE, F_MODRM, F_MODRM,   // PUSH GS, POP GS, RSM, BTS
    F_MODRM|F_IMM8, F_MODRM, F_MODRM, F_MODRM,  // SHRD imm8, SHRD CL, Grp15, IMUL

    // 0xB0-0xBF
    F_MODRM, F_MODRM,            // 0xB0-0xB1 CMPXCHG
    F_MODRM, F_MODRM,            // 0xB2-0xB3 LSS, BTR
    F_MODRM, F_MODRM,            // 0xB4-0xB5 LFS, LGS
    F_MODRM, F_MODRM,            // 0xB6-0xB7 MOVZX r,r/m8 / r,r/m16
    F_INV, F_INV,                // 0xB8 POPCNT(prefix-dep), 0xB9 UD1
    F_MODRM|F_IMM8, F_MODRM,    // 0xBA Grp8 (BT/BTS/BTR/BTC) + imm8, 0xBB BTC
    F_MODRM, F_MODRM,            // 0xBC-0xBD BSF, BSR
    F_MODRM, F_MODRM,            // 0xBE-0xBF MOVSX r,r/m8 / r,r/m16

    // 0xC0-0xCF
    F_MODRM, F_MODRM,            // 0xC0-0xC1 XADD
    F_MODRM|F_IMM8, F_MODRM,    // 0xC2 CMPPS+imm8, 0xC3 MOVNTI
    F_MODRM|F_IMM8, F_MODRM|F_IMM8,  // 0xC4-0xC5 PINSRW+imm8, PEXTRW+imm8
    F_MODRM|F_IMM8, F_MODRM,    // 0xC6 SHUFPS+imm8, 0xC7 Grp9
    F_NONE, F_NONE, F_NONE, F_NONE, F_NONE, F_NONE, F_NONE, F_NONE,  // 0xC8-0xCF BSWAP
    ...
    // 0xF0-0xFF: SSE
    F_MODRM, F_MODRM, F_MODRM, F_MODRM, F_MODRM, F_MODRM, F_MODRM, F_MODRM,
    F_MODRM, F_MODRM, F_MODRM, F_MODRM, F_MODRM, F_MODRM, F_MODRM, F_INV,
};
