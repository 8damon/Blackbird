...
This example shows how to use Stinger's handle-intent correlation and detection telemetry to flag common injection workflows such as process hollowing or manual mapping.

Signal sources:

- `DetectionTelemetry` event with `detectionName=POSSIBLE_PROCESS_HOLLOWING_OR_INJECTION_INTENT_CHAIN`
- `DetectionTelemetry` event with `detectionName=DIRECT_SYSCALL_SUSPECT_HANDLE_OPERATION`
- `DetectionTelemetry` event with `detectionName=SUSPICIOUS_NTDLL_IMAGE_PATH`
- `DetectionTelemetry` event with `detectionName=MULTIPLE_NTDLL_IMAGE_MAPPINGS`

Why it works:

- The driver correlates recent handle intent flags (process-memory + thread-context) and flags a chain that is frequently observed in hollowing and injection pipelines.
...
Minimal flow (ETW consumer using `StingerSensorCore`):
...
- `POSSIBLE_PROCESS_HOLLOWING_OR_INJECTION_INTENT_CHAIN` is emitted when memory+thread intent are observed across a short window.
- `DIRECT_SYSCALL_SUSPECT_HANDLE_OPERATION` is a strong signal when combined with memory intent.
- `SUSPICIOUS_NTDLL_IMAGE_PATH` or `MULTIPLE_NTDLL_IMAGE_MAPPINGS` can indicate manual-mapped ntdll or loader abuse.

To validate in lab:

1. Run `StingerTestSuite.exe` and confirm detection telemetry is observed.
