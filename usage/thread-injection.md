# Example: Detecting Remote Thread Injection

This example shows how to detect remote thread injection using ETW detection events emitted by the driver.

Signal sources:

- `DetectionTelemetry` event with `detectionName=REMOTE_THREAD_WITH_RECENT_HANDLE_INTENT`
- `DetectionTelemetry` event with `detectionName=REMOTE_THREAD_START_IN_NON_IMAGE_EXECUTABLE_REGION`
- `DetectionTelemetry` event with `detectionName=POSSIBLE_MANUAL_MAP_OR_HOLLOWING_EXECUTION`

Prerequisites:

- Driver installed and running.
- Build `StingerSensorCore` and `StingerEtwProc` (or consume ETW via your own tool).
...
- `REMOTE_THREAD_WITH_RECENT_HANDLE_INTENT` indicates a remote thread start correlated with recent handle intent.
- `REMOTE_THREAD_START_IN_NON_IMAGE_EXECUTABLE_REGION` indicates start address is executable but not backed by `MEM_IMAGE`.
- `POSSIBLE_MANUAL_MAP_OR_HOLLOWING_EXECUTION` indicates non-image executable start with memory+thread intent correlation.
