...
static BOOL ControllerEtwTryGetU64Any(_In_ PEVENT_RECORD Record,
...
static BOOL ControllerEtwTryGetU32Any(_In_ PEVENT_RECORD Record,
                                      _In_reads_(NameCount) const PCWSTR *Names,
                                      _In_ size_t NameCount,
                                      _Out_ ULONG *Value)
{
...
static BOOL
ControllerRangesOverlap(_In_ ULONGLONG BaseA, _In_ ULONGLONG SizeA, _In_ ULONGLONG BaseB, _In_ ULONGLONG SizeB)
{
    ULONGLONG endA;
    ULONGLONG endB;
...
static BOOL ControllerProbeManualMapRegion(_In_ DWORD TargetPid,
                                           _In_ ULONGLONG CandidateBase,
                                           _In_ ULONGLONG CandidateSize,
                                           _In_ ULONGLONG ThreadStartAddress,
                                           _Out_ BOOL *PrivateExecutableRegion,
                                           _Out_ BOOL *HasPeHeader,
                                           _Out_ BOOL *ThreadStartInsideRegion,
                                           _Out_opt_ ULONGLONG *ResolvedBase,
                                           _Out_opt_ ULONGLONG *ResolvedSize,
                                           _Out_opt_ ULONG *ResolvedProtect,
...
static PBLACKBIRD_CONTROLLER_HOLLOW_ENTRY
ControllerGetHollowEntryLocked(_In_ DWORD ActorPid, _In_ DWORD TargetPid, _In_ ULONGLONG NowTick)
{
    DWORD i;
...
static VOID ControllerEmitSyntheticDetectionEx(_In_ DWORD ActorPid,
                                               _In_ DWORD TargetPid,
                                               _In_z_ PCSTR DetectionName,
                                               _In_ DWORD Severity,
                                               _In_ UINT64 Marks,
                                               _In_opt_z_ PCWSTR ReasonText)
{
...
static VOID ControllerEmitSyntheticDetection(
        _In_ DWORD ActorPid, _In_ DWORD TargetPid, _In_z_ PCSTR DetectionName, _In_ DWORD Severity, _In_ UINT64 Marks)
{
                                                ...
                                                _In_ ULONGLONG NowTick)
{
    ...
    const BOOL hasThreadSuspicious =
            ((marks
              & (BLACKBIRD_HOLLOW_MARK_THREAD_START_NON_IMAGE_EXEC | BLACKBIRD_HOLLOW_MARK_THREAD_START_OUTSIDE_IMAGE))
             != 0);
    ...
    if (strong && ((NowTick - Entry->LastStrongEmitTick) > 3000ull))
    {
        ...
        if (hasTxf)
        {
            ControllerEmitSyntheticDetection(
                    Entry->ActorPid, Entry->TargetPid, "PROCESS_HOLLOWING_TXF_SUSPECT_CHAIN", 8, Entry->Marks);
            return;
        }
        ControllerEmitSyntheticDetection(
                Entry->ActorPid, Entry->TargetPid, "PROCESS_HOLLOWING_MARK_CHAIN_STRONG", 7, Entry->Marks);
        return;
    }
    ...
    if (medium && ((NowTick - Entry->LastMediumEmitTick) > 3000ull))
    {
        ...
        ControllerEmitSyntheticDetection(
                Entry->ActorPid, Entry->TargetPid, "PROCESS_HOLLOWING_MARK_CHAIN_MEDIUM", 5, Entry->Marks);
    }
}
...
static VOID ControllerTryEmitManualMapDetection(_In_ DWORD ActorPid,
                                                _In_ DWORD TargetPid,
                                                _In_ ULONGLONG BaseHint,
                                                _In_ ULONGLONG SizeHint)
{
    ...
    if (!ControllerProbeManualMapRegion(effectiveTarget,
                                        candidateBase,
                                        candidateSize,
                                        threadStartAddress,
                                        &privateExecutableRegion,
                                        &hasPeHeader,
                                        &threadStartInsideRegion,
                                        &resolvedBase,
                                        &resolvedSize,
                                        &resolvedProtect,
                                        &resolvedType))
    ...
    (void) StringCchPrintfW(
            ...
            resolvedProtect,
            hasPeHeader ? 1u : 0u,
            threadStartInsideRegion ? 1u : 0u,
            marks);
    ControllerEmitSyntheticDetectionEx(effectiveActor, effectiveTarget, detectionName, severity, marks, reasonText);
}

static VOID ControllerApplyHollowMark(_In_ DWORD ActorPid,
                                      _In_ DWORD TargetPid,
                                      _In_ UINT64 Mark,
                                      _In_ ULONGLONG AuxBase,
                                      _In_ ULONGLONG AuxSize,
                                      _In_ ULONG AuxProtect)
{
    PBLACKBIRD_CONTROLLER_HOLLOW_ENTRY entry;
    ULONGLONG nowTick = GetTickCount64();
...
static VOID ControllerHandleBlackbirdHollowRecord(_In_ PEVENT_RECORD Record,
                                                  ...
                                                  _In_ const BLACKBIRD_IPC_ETW_EVENT *BrokerEvent)
{
    ...
    if (wcscmp(EventName, L"ThreadTelemetry") == 0)
    {
        ...
        if (imageBase != 0 && imageSize != 0 && startAddress != 0)
        {
            ...
            {
                ControllerApplyHollowMark((DWORD) creatorPid,
                                          (DWORD) processId,
        ...
        if ((correlationFlags & BLACKBIRD_INTENT_THREAD_CONTEXT) != 0)
        {
            ControllerApplyHollowMark(
                    (DWORD) creatorPid, (DWORD) processId, BLACKBIRD_HOLLOW_MARK_THREAD_CONTEXT_INTENT, 0, 0, 0);
        }
        if (creatorPid != 0 && processId != 0 && creatorPid != processId && correlationFlags != 0)
        {
            ControllerApplyHollowMark(
                    (DWORD) creatorPid, (DWORD) processId, BLACKBIRD_HOLLOW_MARK_REMOTE_THREAD_INTENT, 0, 0, 0);
        }
...
{
    static const PCWSTR callerPidNames[] = {
        L"CallingProcessId", L"CallerProcessId", L"SourceProcessId", L"ProcessId"
    };
    ...
    if (EventName != NULL && EventName[0] != L'\0' && ControllerWideContainsInsensitive(EventName, L"createprocess"))
    {
        ...
        {
            ControllerApplyHollowMark(
                    (DWORD) callerPid, (DWORD) targetPid, BLACKBIRD_HOLLOW_MARK_CREATE_SUSPENDED, 0, 0, 0);
        }
    }
    ...
    switch (task)
    {
        case 1:
            ...
            {
                ControllerApplyHollowMark((DWORD) callerPid,
                                          (DWORD) targetPid,
                                          BLACKBIRD_HOLLOW_MARK_TI_ALLOC_RW_LARGE,
                                          baseAddress,
                                          regionSize,
                                          protect);
            }
        ...
        case 7:
            ...
            ControllerApplyHollowMark((DWORD) callerPid,
                                      (DWORD) targetPid,
                                      BLACKBIRD_HOLLOW_MARK_TI_WRITE_VM,
                                      baseAddress,
                                      regionSize,
                                      0);
        ...
        case 2:
                ...
                && (oldProtect == 0 || ControllerIsWritableProtect(oldProtect)))
            {
                ControllerApplyHollowMark((DWORD) callerPid,
                                          (DWORD) targetPid,
                                          BLACKBIRD_HOLLOW_MARK_TI_PROTECT_RX,
                                          baseAddress,
                                          regionSize,
                                          newProtect);
                ControllerTryEmitManualMapDetection((DWORD) callerPid, (DWORD) targetPid, baseAddress, regionSize);
