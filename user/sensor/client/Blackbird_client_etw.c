...
BOOL WaitForPathLaunchViaEtw(_In_ const BLACKBIRD_TARGET_SPEC *Spec, _Out_ DWORD *Pid)
{
    ...
    BLACKBIRD_ETW_RUN_CONTEXT run;
    BLACKBIRDSC_ETW_SESSION *session = NULL;
    HANDLE etwThread = NULL;
    WCHAR sessionName[64];

    if (Spec == NULL || Spec->Kind != BlackbirdTargetPath || Pid == NULL)
    {
...
static BOOL StartLiveEtw(_Inout_ BLACKBIRD_LIVE_ETW_CONTEXT *Live, _Out_ HANDLE *ThreadHandle)
{
    WCHAR sessionName[64];
    ...
    *ThreadHandle = CreateThread(NULL, 0, LiveEtwRunThreadProc, Live, 0, NULL);
    ...
    {
        BLACKBIRDSCStopEtwSession(Live->Session);
        Live->Session = NULL;
        return FALSE;
    }
...
static DWORD WINAPI BrokerEtwRunThreadProc(_In_ LPVOID Context)
{
    ...
    while (InterlockedCompareExchange(&g_StopRequested, 0, 0) == 0)
    {
        ...
        ok = BLACKBIRDSCGetEtwEvent(broker->Device, &event, 500);
        if (ok)
        {
            if (!BrokerEtwEventMatchesTargetPid(&event, broker->TargetPid, broker->Scope))
            {
                continue;
            }
                    ...
                    _In_ DWORD StreamMask, _In_ BLACKBIRD_TARGET_SCOPE Scope)
{
    ...
    *ThreadHandle = NULL;
    ...
    Broker->Device = BLACKBIRDSCOpenControlDevice();
    ...
    Broker->TiEnableError = BLACKBIRDSCGetBrokerThreatIntelEnableError();
    Broker->TargetPid = SeedPid;
    Broker->Scope = Scope;
