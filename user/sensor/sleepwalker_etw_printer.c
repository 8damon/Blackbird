...
typedef struct _SLEEPWALKER_PID_IMAGE_CACHE_ENTRY
{
    ULONGLONG Pid;
    WCHAR Path[SLEEPWALKER_MAX_IMAGE_PATH_CHARS];
} SLEEPWALKER_PID_IMAGE_CACHE_ENTRY;
...
typedef struct _SLEEPWALKER_REGISTRY_SUPPRESSOR
{
    BOOL Active;
    ULONGLONG ProcessId;
    ULONG SessionId;
...
} SLEEPWALKER_REGISTRY_SUPPRESSOR;

static SLEEPWALKER_REGISTRY_SUPPRESSOR g_RegistrySuppressor;

static BOOL SLEEPWALKERRegistryIsSuppressible(_In_z_ const CHAR *Operation, _In_ BOOL HighValue)
{
    if (Operation == NULL)
    {
        return FALSE;
...
static BOOL SLEEPWALKERRegistrySuppressorMatches(_In_ const SLEEPWALKER_REGISTRY_SUPPRESSOR *State,
...
static VOID SLEEPWALKERFlushRegistrySuppressor(VOID)
{
...
static VOID SLEEPWALKERUpdateRegistrySuppressor(_In_z_ const CHAR *Operation, _In_ ULONGLONG ProcessId,
                                                _In_ ULONG SessionId, _In_ ULONG NotifyClass, _In_ ULONG DataType,
                                                _In_ ULONG DataSize, _In_ BOOL HighValue,
                                                _In_z_ const WCHAR *ProcessImage, _In_z_ const WCHAR *KeyPath,
                                                _In_z_ const WCHAR *ValueName)
...
static BOOL SLEEPWALKERPathEndsWithInsensitive(_In_z_ PCWSTR Path, _In_z_ PCWSTR Suffix)
{
    size_t pathLen;
    size_t suffixLen;
...
static BOOL SLEEPWALKERIsExecutableImagePath(_In_z_ PCWSTR Path)
{
    ...
    return SLEEPWALKERPathEndsWithInsensitive(Path, L".exe") || SLEEPWALKERPathEndsWithInsensitive(Path, L".com");
}

static BOOL SLEEPWALKERLookupCachedProcessImage(_In_ ULONGLONG Pid, _Out_writes_z_(OutputChars) PWSTR Output,
...
static VOID SLEEPWALKERCacheProcessImage(_In_ ULONGLONG Pid, _In_opt_z_ PCWSTR Path)
{
    ULONG i;

    if (Pid == 0 || Path == NULL || Path[0] == L'\0' || Path[0] == L'<' || !SLEEPWALKERIsExecutableImagePath(Path))
    {
        return;
    }
...
static BOOL SLEEPWALKERQueryProcessImageFromKernel(_In_ ULONGLONG Pid, _Out_writes_z_(OutputChars) PWSTR Output,
                                                   _In_ size_t OutputChars)
{
    HANDLE device;
...
{
    SLEEPWALKERCacheProcessImage(Pid, ImagePath);
}
...
void SLEEPWALKERPrimeProcessImageFromEtw(_In_ PEVENT_RECORD Record, _In_opt_z_ PCWSTR EventName)
{
    ...
    imagePath[0] = L'\0';
    if (!SLEEPWALKERGetU64Property(Record, L"processId", &processId) || processId == 0)
    {
        return;
    }
    if (!SLEEPWALKERGetWideProperty(Record, L"imagePath", imagePath, RTL_NUMBER_OF(imagePath)))
    {
        return;
    }

    SLEEPWALKERCacheProcessImage(processId, imagePath);
    if (wcscmp(EventName, L"ImageTelemetry") == 0)
    {
        ULONGLONG imageBase = 0;
        ULONGLONG imageSize = 0;
        if (SLEEPWALKERGetU64Property(Record, L"imageBase", &imageBase) &&
            SLEEPWALKERGetU64Property(Record, L"imageSize", &imageSize) && imageBase != 0 && imageSize != 0)
        {
            SLEEPWALKEREtwSymbolsCacheModuleForProcess((DWORD)processId, imageBase, imageSize, imagePath);
        }
    }
}
...
static void FormatCorrelationFlags(_In_ ULONG Flags, _Out_writes_z_(OutputChars) PWSTR Output, _In_ size_t OutputChars)
{
    ...
    if ((Flags & SLEEPWALKER_INTENT_PROCESS_MEMORY) != 0)
    {
        AppendFlag(Output, OutputChars, L"ProcessMemory", &first);
    }
    if ((Flags & SLEEPWALKER_INTENT_THREAD_CONTEXT) != 0)
    {
        AppendFlag(Output, OutputChars, L"ThreadContext", &first);
    }
    if ((Flags & SLEEPWALKER_INTENT_DUP_HANDLE) != 0)
    {
...
static void FormatProcessImage(_In_ ULONGLONG Pid, _Out_writes_z_(OutputChars) PWSTR Output, _In_ size_t OutputChars)
{
    ...
    if (SLEEPWALKERLookupCachedProcessImage(Pid, Output, OutputChars))
    {
        return;
    }
    ...
    if (process != NULL)
    {
        ...
        {
            SLEEPWALKERCacheProcessImage(Pid, Output);
            CloseHandle(process);
            return;
        }
        CloseHandle(process);
    ...
    if (SLEEPWALKERQueryProcessImageFromKernel(Pid, Output, OutputChars) && Output[0] != L'\0')
    {
        SLEEPWALKERCacheProcessImage(Pid, Output);
        return;
    }
...
static BOOL SLEEPWALKERIsLikelyMainImageAddress(_In_ ULONGLONG Address)
{
                                       ...
                                       _Out_writes_z_(OutputChars) PWSTR Output, _In_ size_t OutputChars)
{
    ...
    SLEEPWALKEREtwSymbolsFormatAddressForProcess(ProcessId, Address, Output, OutputChars);
    ...
    if (ProcessId != 0 && SLEEPWALKEREtwSymbolsTryResolveViaModuleCache(ProcessId, Address, Output, OutputChars))
    {
        return;
    }
    if (!SLEEPWALKERIsLikelyMainImageAddress(Address))
    {
        return;
    }
                       ...
                       _In_ DWORD SecondaryProcessId, _In_opt_z_ PCWSTR ImageHint)
{
    ...
    for (i = 0; i < Count && i < 8; ++i)
    {
        ...
        if (!SLEEPWALKERGetU64Property(Record, name, &addr))
        {
            continue;
        }
...
static void PrintHandleTelemetry(_In_ PEVENT_RECORD Record)
{
    ...
    BYTE deepSample[SLEEPWALKER_MAX_DEEP_SAMPLE_BYTES];
    ...
    (void)SLEEPWALKERGetAnsiProperty(Record, L"class", eventClass, RTL_NUMBER_OF(eventClass));
    (void)SLEEPWALKERGetU64Property(Record, L"callerPid", &callerPid);
    (void)SLEEPWALKERGetU64Property(Record, L"targetPid", &targetPid);
    (void)SLEEPWALKERGetU32Property(Record, L"desiredAccess", &desiredAccess);
    (void)SLEEPWALKERGetU64Property(Record, L"originAddress", &originAddress);
    (void)SLEEPWALKERGetU32Property(Record, L"originProtect", &originProtect);
    (void)SLEEPWALKERGetBoolProperty(Record, L"execProtect", &execProtect);
    (void)SLEEPWALKERGetBoolProperty(Record, L"fromNtdll", &fromNtdll);
    (void)SLEEPWALKERGetBoolProperty(Record, L"fromExe", &fromExe);
    (void)SLEEPWALKERGetWideProperty(Record, L"originPath", path, RTL_NUMBER_OF(path));
    (void)SLEEPWALKERGetU32Property(Record, L"frameCount", &frameCount);
    (void)SLEEPWALKERGetI32Property(Record, L"statusOpenProcess", &statusOpen);
    (void)SLEEPWALKERGetI32Property(Record, L"statusBasicInfo", &statusBasic);
    (void)SLEEPWALKERGetI32Property(Record, L"statusSectionName", &statusSection);
    (void)SLEEPWALKERGetU64Property(Record, L"deepAllocationBase", &deepAllocationBase);
    (void)SLEEPWALKERGetU64Property(Record, L"deepRegionSize", &deepRegionSize);
    (void)SLEEPWALKERGetU32Property(Record, L"deepRegionProtect", &deepRegionProtect);
    (void)SLEEPWALKERGetU32Property(Record, L"deepRegionState", &deepRegionState);
    (void)SLEEPWALKERGetU32Property(Record, L"deepRegionType", &deepRegionType);
    if (!SLEEPWALKERGetBinaryProperty(Record, L"deepSample", deepSample, sizeof(deepSample), &deepSampleSize))
    {
        deepSampleSize = 0;
    }
...
static void PrintThreadTelemetry(_In_ PEVENT_RECORD Record)
{
    ...
    (void)SLEEPWALKERGetU64Property(Record, L"processId", &processId);
    (void)SLEEPWALKERGetU64Property(Record, L"threadId", &threadId);
    (void)SLEEPWALKERGetU64Property(Record, L"creatorPid", &creatorPid);
    (void)SLEEPWALKERGetU64Property(Record, L"startAddress", &startAddress);
    (void)SLEEPWALKERGetU64Property(Record, L"imageBase", &imageBase);
    (void)SLEEPWALKERGetU64Property(Record, L"imageSize", &imageSize);
    (void)SLEEPWALKERGetBoolProperty(Record, L"gotStart", &gotStart);
    (void)SLEEPWALKERGetBoolProperty(Record, L"gotRange", &gotRange);
    (void)SLEEPWALKERGetBoolProperty(Record, L"isRemoteCreator", &isRemote);
    (void)SLEEPWALKERGetBoolProperty(Record, L"outsideMainImage", &outsideImage);
    (void)SLEEPWALKERGetU32Property(Record, L"correlationFlags", &correlationFlags);
    (void)SLEEPWALKERGetU32Property(Record, L"correlationAccessMask", &correlationAccessMask);
    (void)SLEEPWALKERGetU32Property(Record, L"correlationAgeMs", &correlationAgeMs);
    (void)SLEEPWALKERGetU32Property(Record, L"startRegionProtect", &startRegionProtect);
    (void)SLEEPWALKERGetU32Property(Record, L"startRegionState", &startRegionState);
    (void)SLEEPWALKERGetU32Property(Record, L"startRegionType", &startRegionType);
    (void)SLEEPWALKERGetI32Property(Record, L"startRegionStatus", &startRegionStatus);
    (void)SLEEPWALKERGetU32Property(Record, L"workerFrameCount", &frameCount);
...
static void PrintProcessTelemetry(_In_ PEVENT_RECORD Record)
{
    ...
    (void)SLEEPWALKERGetBoolProperty(Record, L"isCreate", &isCreate);
    (void)SLEEPWALKERGetI32Property(Record, L"createStatus", &createStatus);
    (void)SLEEPWALKERGetU64Property(Record, L"processId", &processId);
    (void)SLEEPWALKERGetU64Property(Record, L"parentProcessId", &parentPid);
    (void)SLEEPWALKERGetU64Property(Record, L"creatorProcessId", &creatorPid);
    (void)SLEEPWALKERGetU64Property(Record, L"creatorThreadId", &creatorTid);
    (void)SLEEPWALKERGetU64Property(Record, L"processStartKey", &startKey);
    (void)SLEEPWALKERGetU32Property(Record, L"sessionId", &sessionId);
    (void)SLEEPWALKERGetWideProperty(Record, L"imagePath", imagePath, RTL_NUMBER_OF(imagePath));
    (void)SLEEPWALKERGetWideProperty(Record, L"commandLine", commandLine, RTL_NUMBER_OF(commandLine));
    SLEEPWALKERCacheProcessImage(processId, imagePath);
...
static void PrintImageTelemetry(_In_ PEVENT_RECORD Record)
{
    ...
    (void)SLEEPWALKERGetU64Property(Record, L"imageBase", &imageBase);
    (void)SLEEPWALKERGetU64Property(Record, L"imageSize", &imageSize);
    (void)SLEEPWALKERGetBoolProperty(Record, L"isSystemModeImage", &systemMode);
    (void)SLEEPWALKERGetBoolProperty(Record, L"isSignatureLevelKnown", &sigKnown);
    (void)SLEEPWALKERGetU8Property(Record, L"signatureLevel", &sigLevel);
    (void)SLEEPWALKERGetU8Property(Record, L"signatureType", &sigType);
    (void)SLEEPWALKERGetWideProperty(Record, L"imagePath", imagePath, RTL_NUMBER_OF(imagePath));
    SLEEPWALKERCacheProcessImage(processId, imagePath);
    SLEEPWALKEREtwSymbolsCacheModuleForProcess((DWORD)processId, imageBase, imageSize, imagePath);

    SLEEPWALKEREtwSymbolsFormatAddressForProcess((DWORD)processId, imageBase, imageSym, RTL_NUMBER_OF(imageSym));
    FormatProcessImage(processId, processImage, RTL_NUMBER_OF(processImage));
...
static void PrintRegistryTelemetry(_In_ PEVENT_RECORD Record)
{
    ...
    (void)SLEEPWALKERGetAnsiProperty(Record, L"operation", operation, RTL_NUMBER_OF(operation));
    (void)SLEEPWALKERGetU64Property(Record, L"processId", &processId);
    (void)SLEEPWALKERGetU32Property(Record, L"sessionId", &sessionId);
    (void)SLEEPWALKERGetU32Property(Record, L"notifyClass", &notifyClass);
    (void)SLEEPWALKERGetU32Property(Record, L"dataType", &dataType);
    (void)SLEEPWALKERGetU32Property(Record, L"dataSize", &dataSize);
    (void)SLEEPWALKERGetBoolProperty(Record, L"isHighValuePath", &highValue);
    (void)SLEEPWALKERGetWideProperty(Record, L"keyPath", keyPath, RTL_NUMBER_OF(keyPath));
    (void)SLEEPWALKERGetWideProperty(Record, L"valueName", valueName, RTL_NUMBER_OF(valueName));

    FormatProcessImage(processId, processImage, RTL_NUMBER_OF(processImage));
    suppressible = SLEEPWALKERRegistryIsSuppressible(operation, highValue);

    if (suppressible &&
        SLEEPWALKERRegistrySuppressorMatches(&g_RegistrySuppressor, operation, processId, sessionId, notifyClass,
    ...
    SLEEPWALKERFlushRegistrySuppressor();
    ...
    if (suppressible)
    {
        SLEEPWALKERUpdateRegistrySuppressor(operation, processId, sessionId, notifyClass, dataType, dataSize, highValue,
...
static void PrintDetectionTelemetry(_In_ PEVENT_RECORD Record)
{
    ...
    (void)SLEEPWALKERGetAnsiProperty(Record, L"detectionName", detectionName, RTL_NUMBER_OF(detectionName));
    (void)SLEEPWALKERGetU32Property(Record, L"severity", &severity);
    (void)SLEEPWALKERGetU64Property(Record, L"processId", &processId);
    (void)SLEEPWALKERGetU64Property(Record, L"targetPid", &targetPid);
    (void)SLEEPWALKERGetU32Property(Record, L"correlationFlags", &correlationFlags);
    (void)SLEEPWALKERGetU32Property(Record, L"correlationAccessMask", &correlationAccessMask);
    (void)SLEEPWALKERGetU32Property(Record, L"correlationAgeMs", &correlationAgeMs);
    (void)SLEEPWALKERGetWideProperty(Record, L"reason", reason, RTL_NUMBER_OF(reason));
...
static BOOL SLEEPWALKERTryGetU64Any(_In_ PEVENT_RECORD Record, _In_reads_(NameCount) const PCWSTR *Names,
                                    _In_ size_t NameCount, _Out_ ULONGLONG *Value)
{
    ...
    *Value = 0;
    ...
    {
        if (SLEEPWALKERGetU64Property(Record, Names[i], Value))
        {
            return TRUE;
        }
    }
...
static BOOL SLEEPWALKERTryGetU32Any(_In_ PEVENT_RECORD Record, _In_reads_(NameCount) const PCWSTR *Names,
                                    _In_ size_t NameCount, _Out_ ULONG *Value)
{
    ...
    *Value = 0;
    ...
    for (i = 0; i < NameCount; ++i)
    {
        if (SLEEPWALKERGetU32Property(Record, Names[i], Value))
        {
            return TRUE;
        }
    }
...
void SLEEPWALKERPrintThreatIntelRecord(_In_ PEVENT_RECORD Record, _In_opt_z_ PCWSTR EventName)
{
    ...
    (void)SLEEPWALKERTryGetU64Any(Record, callerPidNames, RTL_NUMBER_OF(callerPidNames), &callerPid);
    (void)SLEEPWALKERTryGetU64Any(Record, targetPidNames, RTL_NUMBER_OF(targetPidNames), &targetPid);
    (void)SLEEPWALKERTryGetU64Any(Record, targetTidNames, RTL_NUMBER_OF(targetTidNames), &targetTid);
    (void)SLEEPWALKERTryGetU64Any(Record, startAddrNames, RTL_NUMBER_OF(startAddrNames), &startAddress);
    (void)SLEEPWALKERTryGetU64Any(Record, routineNames, RTL_NUMBER_OF(routineNames), &routineAddress);
    (void)SLEEPWALKERTryGetU32Any(Record, accessNames, RTL_NUMBER_OF(accessNames), &desiredAccess);
    ...
    nameSuggestsCreateThread = SLEEPWALKEREventNameContainsInsensitive(safeName, L"createthread") ||
                               SLEEPWALKEREventNameContainsInsensitive(safeName, L"ntcreatethreadex");
    nameSuggestsSetContext = SLEEPWALKEREventNameContainsInsensitive(safeName, L"setcontext") ||
                             SLEEPWALKEREventNameContainsInsensitive(safeName, L"setthreadcontext");
    nameSuggestsQueueApc = SLEEPWALKEREventNameContainsInsensitive(safeName, L"queueapc") ||
                           SLEEPWALKEREventNameContainsInsensitive(safeName, L"insertqueueapc");
