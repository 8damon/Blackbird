...
static BOOL SLEEPWALKERIsLikelyKernelAddress(UINT64 address)
{
#if defined(_WIN64)
...
static void SLEEPWALKERLoadKernelModules(void)
{
    ...
    status = SLEEPWALKERSymLoadKernelModulesForProcess(g_SymbolProcess, &loaded, &count);
...
void SLEEPWALKERSymbolResolverInitialize(void)
{
    ...
    {
        (void)SymSetSearchPath(g_SymbolProcess, SLEEPWALKER_DEFAULT_SYMBOL_PATH);
    }
...
void SLEEPWALKERSymbolResolverPrintAddress(UINT64 address)
{
    ...
    if (!g_SymbolsReady)
    {
        if (SLEEPWALKERIsLikelyKernelAddress(address))
        {
