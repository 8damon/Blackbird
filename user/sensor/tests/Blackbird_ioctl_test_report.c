...
BOOL SuiteInitReport(_Inout_ SUITE_RESULTS *Results)
{
    ...
    fprintf(Results->Report, "BlackbirdTestSuite report\n");
                  ...
                  _In_z_ const char *FailText)
{
    ...
    {
        Results->Passed += 1;
        SuiteRecordCheck(Results, BlackbirdReportCheckPass, PassText);
    }
    else
    {
        SuiteRecordCheck(Results, BlackbirdReportCheckFail, FailText);
    }
}
